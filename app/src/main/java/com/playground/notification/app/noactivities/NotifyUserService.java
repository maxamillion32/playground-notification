...
public class NotifyUserService extends IntentService implements LocationListener {
	...
	private void notify( String title, String desc, PendingIntent contentIntent ) {
		android.support.v4.app.NotificationCompat.Builder notifyBuilder = new NotificationCompat.Builder( this ).setWhen( System.currentTimeMillis() )
				.setSmallIcon( R.drawable.ic_balloon ).setTicker( title ).setContentTitle( title ).setContentText( desc ).setStyle(
						...
						public void onConnected( Bundle bundle ) {
							if( mGoogleApiClient != null && mGoogleApiClient.isConnected() && mLocationRequest != null ) {
								LocationServices.FusedLocationApi.requestLocationUpdates(
										mGoogleApiClient, mLocationRequest, NotifyUserService.this );
							}
						}
	...
	public void onLocationChanged( Location location ) {
		...
		try {
			Api.getWeather( location.getLatitude(), location.getLongitude(), Locale.getDefault().getLanguage(), units, App.Instance.getWeatherKey(),
									...
									if( details != null && details.size() > 0 ) {
										...
										if( weatherDetail != null ) {
											...
											if( isGoodWeatherCondition( weatherDetail ) ) {
												PendingIntent pi = PendingIntent.getActivity( App.Instance, (int) System.currentTimeMillis(), i,
																							  PendingIntent.FLAG_UPDATE_CURRENT
												);
												NotifyUserService.this.notify(
														title, App.Instance
																.getString( R.string.notify_content, weatherDetail.getDescription(), temp ), pi );
											}
										}
									}
									//Ignore...
